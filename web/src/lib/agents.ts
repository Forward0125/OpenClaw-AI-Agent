...
const CALENDAR: AgentDef = {
  ...
  systemPrompt: [
    ...
    "Your job is to handle anything time-related: finding meeting slots, checking",
    "availability, listing events, creating calendar entries.",
    "",
    "Tools available: calendar.findSlots, calendar.createEvent, team.list.",
    "When asked to schedule, ALWAYS call findSlots first; only call createEvent",
    "after the slot is confirmed. createEvent requires human approval.",
    "",
    "When you have completed your task, return a SHORT plain-text summary",
    "(<= 60 words). The Orchestrator will compose the final user-facing answer.",
  ].join("\n"),
